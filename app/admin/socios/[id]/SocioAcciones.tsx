...
export default function SocioAcciones({ socioId }: { socioId: string }) {
  ...
  async function onMarkDone(accionId: string) {
    ...
    startTransition(async () => {
      try {
        const res = await fetch(`/api/admin/socios/${socioId}/acciones/${accionId}`, {
          method: "PATCH",
          cache: "no-store",
          headers: { "Cache-Control": "no-store" },
        });
    ...
    <div className="mt-6 rounded-2xl border bg-white p-6">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          disabled={isPending || !fechaLimite.trim()}
          className="rounded-xl border px-4 py-2 text-sm hover:bg-slate-50 disabled:cursor-not-allowed"
        >
          + Email
        </button>
